...
namespace BloodCenterManagementSystem.DataAccess
{
    public class DonationRepository: Repository<DonationModel>, IDonationRepository
    {
        ...
        {
            return DataContext.Donations.Include(m => m.BloodDonator).Include(m => m.BloodDonator.User).Include(m=>m.ResultOfExamination).Include(m=>m.BloodDonator.BloodType).Include(m=>m.BloodStorage).FirstOrDefault(m => m.Id == id);
        }
